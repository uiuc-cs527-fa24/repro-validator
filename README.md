...
``` sh
nix run github:uiuc-cs527-fa24/repro-validator/0.2.18 -- validate bibcode.yaml

# or

docker run --rm -t -e TERM=$TERM -e CHOWN=1 -e UID=$(id -u) -e GID=$(id -g) -v $PWD:$PWD -w $PWD ghcr.io/charmoniumq/repro-validator:0.2.18 validate bibcode.yaml
```
...
- Validator correctly identifies archive.org URLs now.
